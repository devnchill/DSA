...
days like this: 1st day: 3, 2 2nd day: 2, 4 3rd day: 1, 4
...
public:
  int shipWithinDays(vector<int> &weights, int days) {
    ...
    while (low <= high) {
      ...
      int required_days = total_days(weights, mid);
...
private:
  int total_days(vector<int> &nums, int weight) {
    int curr_weight = 0;
    int day = 1;
    for (auto it : nums) {
      if (curr_weight + it > weight) {
        curr_weight = it;
        day++;
      } else {
        curr_weight += it;
...
int main() {
  ...
  std::cout << s.shipWithinDays(nums, 4);
  return 0;
}
