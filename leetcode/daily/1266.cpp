...
unit horizontally in 1 second). You have to visit the points in the same order
as they appear in the array. You are allowed to pass through points that appear
...
public:
  int minTimeToVisitAllPoints(vector<vector<int>> &points) {
    ...
    for (int i = 0; i < points.size() - 1; i++) {
      sum += max(abs(points[i + 1][0] - points[i][0]),
