...
3 + 4) = 37. Notice that on the 2nd Monday, Hercy only puts in $2. Example 3:
...
public:
  int totalMoney(int n) {
    ...
    int i = 1;
    while (i <= weeks) {
      int start_day_money = i;
      int end_day_money = i + 6;
      int curr_week_sum = (end_day_money * (end_day_money + 1) / 2) -
                          ((start_day_money - 1) * (start_day_money) / 2);
      sum += curr_week_sum;
      i++;
    }

    int curr_day_money = weeks + 1;
    sum +=
        (days) * (days + 1) / 2 - ((curr_day_money) * (curr_day_money - 1) / 2);
