...
// in cells (0, 0) and (n - 1, n - 1). Example 2:
//
//
...
// The distance between them is | 0 - 0 | + | 0 - 2 | = 2. It can be shown that
// there are no other paths with a higher safeness factor. Example 3:
//
//
...
// The distance between them is | 3 - 3 | + | 0 - 2 | = 2. It can be shown that
// there are no other paths with a higher safeness factor.
//
//
// Constraints:
//
...
private:
  int ans = 0;
  vector<pair<int, int>> thief_pos;
  vector<vector<bool>> visited;

  void dfs(vector<vector<int>> &grid, int r, int c, int currentMin) {
    int n = grid.size();
    ...
    int nearest = INT_MAX;
    for (auto [x, y] : thief_pos) {
      nearest = min(nearest, abs(x - r) + abs(y - c));
    }

    currentMin = min(currentMin, nearest);
    ...
    dfs(grid, r + 1, c, currentMin);
    dfs(grid, r - 1, c, currentMin);
    dfs(grid, r, c + 1, currentMin);
    dfs(grid, r, c - 1, currentMin);
...
public:
  int maximumSafenessFactor(vector<vector<int>> &grid) {
    ...
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n; j++) {
        if (grid[i][j] == 1)
          thief_pos.push_back({i, j});
      }
    }

    dfs(grid, 0, 0, INT_MAX);
