...
// can be all found as substrings at indices 0, 1, 3 and 2 respectively. Example
// 2:
//
// Input: s = "0110", k = 1
// Output: true
// Explanation: The binary codes of length 1 are "0" and "1", it is clear that
// both exist as a substring. Example 3:
//
// Input: s = "0110", k = 2
// Output: false
...
class Solution {
private:
  unordered_set<string> is_substring;
  bool backtrack(string &s, int i) {
    if (i == s.size())
      return is_substring.count(s);
    s[i] = '0';
    if (!backtrack(s, i + 1))
      return false;
    s[i] = '1';
...
public:
  bool hasAllCodes(string s, int k) {
    for (int i = 0; i + k <= s.size(); i++) {
      is_substring.insert(s.substr(i, k));
    }
    string original_binary_to_check(k, '0');
    return backtrack(original_binary_to_check, 0);
  }
};
