...
// for each building. We can build the buildings with heights [0,1,2,1,2], and
// the tallest building has a height of 2. Example 2:
//
//
...
// the tallest building has a height of 5. Example 3:
//
//
...
public:
  int maxBuilding(int n, vector<vector<int>> &restrictions) {
    ...
    int prev_height = 0;
    for (int i = 1; i < n; i++) {
      int lower = prev_height - 1;
      int upper = prev_height + 1;
      if (m.count(i))
        upper = min(upper, m[i]);
      max_height = max(max_height, upper);
    }
    return max_height;
  }
};
