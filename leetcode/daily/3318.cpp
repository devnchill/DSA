...
public:
  vector<int> findXSum(vector<int> &nums, int k, int x) {
    ...
    for (int i = 0; i + k - 1 < n; i++) {
      ...
      vector<pair<int, int>> elemWithFreq = getElemWithFreq(subArr);
      res.push_back(getSumOfXMaxElem(x, elemWithFreq));
    }
    return res;
  }

private:
  vector<pair<int, int>> getElemWithFreq(const vector<int> &arr) {
  ...
  int getSumOfXMaxElem(int x, const vector<pair<int, int>> &arr) {
    if (arr.size() < x) {
...
int main() {
  ...
  vector<int> res = s.findXSum(nums, 2, 2);
  for (int val : res)
    cout << val << " ,";
  return 0;
}
