...
// and '%'. 0 <= k <= 1015 The length of result after processing s will not
// exceed 1015.
...
public:
  char processStr(string s, long long k) {
    long long len = 0;
    // simulate just to check if len < k+1 as for that we'll just return '.'
    for (auto c : s) {
      if (c == '*') {
        if (len) {
          len--;
    ...
    // now here on it means len > k+1, so we'll reverse operation and after each
    // check if len gets equal to k+1 , if yes we could return s[i]
