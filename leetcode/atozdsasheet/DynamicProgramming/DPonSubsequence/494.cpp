...
class Solution {
private:
  int dfs(int index, int target, int curr_sum, vector<int> &nums) {
    if (index < 0 && curr_sum == target)
      return 1;
    if (index < 0)
      return 0;
    int positive = dfs(index - 1, target, curr_sum + nums[index], nums);
    int negative = dfs(index - 1, target, curr_sum + (-nums[index]), nums);
    return positive + negative;
  }

public:
  int findTargetSumWays(vector<int> &nums, int target) {
    return dfs(nums.size() - 1, target, 0, nums);
  }
};
