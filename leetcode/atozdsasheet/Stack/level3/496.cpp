...
public:
  vector<int> nextGreatElement(vector<int> &nums1, vector<int> &nums2) {
    ...
    for (size_t i = 0; i < n1; i++) {
      vector<int>::iterator target = find(nums2.begin(), nums2.end(), nums1[i]);
      size_t index = target - nums2.begin();
      for (size_t j = index + 1; j < n2; j++) {
...
int main() {
  ...
  vector<int> res = s.nextGreatElement(nums1, nums2);
  cout << "[";
  for (int val : res) {
    cout << val << ",";
  }
