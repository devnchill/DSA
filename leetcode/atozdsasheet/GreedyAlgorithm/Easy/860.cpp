...
bill. For the last customer, we can not give the change of $15 back because we
only have two $10 bills. Since not every customer received the correct change,
the answer is false.
...
public:
  bool lemonadeChange(vector<int> &bills) {
    int freq_five = 0, freq_ten = 0, freq_twenty = 0;
    for (int &val : bills) {
      switch (val) {
      case 5: {
        freq_five++;
