...
in their original order. If there are no non-zero digits, x = 0. Let sum be the
sum of digits in x. The answer is x * sum. Return an array of integers answer
...
public:
  vector<int> sumAndMultiply(string s, vector<vector<int>> &queries) {
    ...
    vector<int> res;
    for (auto &p : queries) {
      string curr = currentstring.substr(p[0], p[1] - p[0] + 1);
      long long sum = 0;
      ...
      for (char ch : curr) {
        ...
        if (d != 0) {
          sum = (sum + d) % MOD;
          newNumber = (newNumber * 10 + d) % MOD;
        }
      }
      long long ans = (sum * newNumber) % MOD;
