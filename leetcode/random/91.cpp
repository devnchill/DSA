...
// "6" is valid). Note: there may be strings that are impossible to decode.
//
...
private:
  ...
  int rec(string &s, int i, bool is_carry) {
    if (dp[i][is_carry] != -1)
      return dp[i][is_carry];
    if (i == s.size())
      return dp[i][is_carry] = !is_carry;
    if (is_carry) {
      int num = (s[i - 1] - '0') * 10 + (s[i] - '0');
      if (num < 10 || num > 26)
        return dp[i][is_carry] = 0;
      return dp[i][is_carry] = rec(s, i + 1, false);
    }
    if (s[i] == '0')
      return dp[i][is_carry] = 0;
    int ans = rec(s, i + 1, false);
    if (i + 1 < s.size())
      ans += rec(s, i + 1, true);
    return dp[i][is_carry] = ans;
  }

public:
  int numDecodings(string s) { return rec(s, 0, false); }
};
